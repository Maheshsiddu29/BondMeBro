...
contract BondThresholdsTest is Test, Deployers {
    ...
    uint128 internal constant GENEROUS_CEILING =
        type(uint128).max;
    ...
    function setUp() public {
        ...
        (token0, token1) =
            address(sixDec) < address(eighteenDec)
                ? (sixDec, eighteenDec)
                : (eighteenDec, sixDec);
        ...
        assertTrue(
            dec0 != dec1,
            "the whole point of this suite is asymmetric decimals"
        );
    ...
    function test_sameNominalAmount_bondsOneDirectionOnly()
        public
    {
        uint256 bond0 =
            _bondTakenBy(
                -int256(STRADDLING_AMOUNT),
                true,
                _validHookData()
            );
        ...
        assertGt(
            bond0,
            0,
            "currency0-in swap above its threshold was not bonded"
        );
        ...
        assertEq(
            ...
            STRADDLING_AMOUNT *
                BOND_BPS /
                10_000,
            "currency0 bond is not bondBps of gross"
        );
        ...
        assertEq(
            bond1,
            0,
            "currency1-in swap below its threshold was bonded"
        );
    ...
    function test_exactInput_zeroForOne_selectsThreshold0()
        public
    {
        assertEq(
            _bondTakenBy(
                -int256(
                    uint256(THRESHOLD_0) - 1
                ),
                true,
                ""
            ),
            0,
            "one wei below threshold0 was bonded"
        );
        ...
        assertEq(
            ...
            uint256(THRESHOLD_0) *
                BOND_BPS /
                10_000,
            "exactly at threshold0 was not bonded correctly"
        );
    }

    /// @notice For oneForZero exact-input swaps, currency1 is the input and `THRESHOLD_1` must be used.
    function test_exactInput_oneForZero_selectsThreshold1()
        public
    {
        assertEq(
            _bondTakenBy(
        ...
        assertEq(
            ...
            uint256(THRESHOLD_1) *
                BOND_BPS /
                10_000,
            "exactly at threshold1 was not bonded correctly"
        );
    }
    ...
    function test_exactOutput_zeroForOne_selectsThreshold0()
        public
    {
        uint256 bond =
            _bondTakenBy(
        ...
        assertGt(
            bond,
            0,
            "exact-output currency0-in above threshold0 was not bonded"
        );
    }
    ...
    function test_exactOutput_oneForZero_selectsThreshold1()
        public
    {
        uint256 bond =
            _bondTakenBy(
        ...
        assertEq(
            bond,
            0,
            "exact-output currency1-in below threshold1 was bonded"
        );
    }

    /// @notice The currency1 exact-output path starts bonding once its own threshold is exceeded.
    function test_exactOutput_oneForZero_bondsAboveThreshold1()
        public
    {
        uint256 bond =
            _bondTakenBy(
                int256(
                    uint256(THRESHOLD_1) * 2
                ),
                false,
                _validHookData()
            );
        ...
        assertGt(
            bond,
            0,
            "exact-output currency1-in above threshold1 was not bonded"
        );
    }
    ...
    function test_setPoolConfig_acceptsMaxUint96Threshold()
        public
    {
        hook.setPoolConfig(
            key_,
        ...
        assertEq(
            min1,
            type(uint96).max,
            "uint96 threshold was truncated"
        );

        assertEq(
            min0,
            type(uint128).max,
            "neighbouring uint128 field was corrupted"
        );

        assertEq(
            bondBps,
            BOND_BPS,
            "neighbouring uint16 field was corrupted"
        );
    }
    ...
    function test_setPoolConfig_rejectsOversizedThresholdViaRawCalldata()
        public
    {
        uint256 tooBig =
        ...
        assertFalse(
            success,
            "an oversized uint96 threshold was accepted through raw calldata"
        );
        ...
        assertEq(
            min0,
            THRESHOLD_0,
            "config was mutated by a rejected call"
        );
    ...
    function test_setPoolConfig_rawCalldataWithinRangeSucceeds()
        public
    {
        bytes memory goodCalldata =
            abi.encodeWithSelector(
                BondMeBro.setPoolConfig.selector,
                key_,
                uint256(THRESHOLD_0),
        ...
        assertTrue(
            success,
            "a valid raw-calldata config was rejected"
        );
        ...
        assertEq(
            min1,
            type(uint96).max,
            "raw-calldata config did not take effect"
        );
    }
    ...
    function test_configOccupiesOneSlot_exactInput()
        public
    {
        _assertConfigTouchesOnlyItsOwnSlot(
            -int256(STRADDLING_AMOUNT)
        );
    }

    function test_configOccupiesOneSlot_exactOutput()
        public
    {
        _assertConfigTouchesOnlyItsOwnSlot(
            int256(STRADDLING_AMOUNT)
    ...
    function _assertConfigTouchesOnlyItsOwnSlot(
        int256 amountSpecified
    )
        internal
    {
        ...
        assertGt(
            ...
            0,
            "the config slot was never read"
        );
        ...
        assertEq(
            ...
            0,
            "config spilled into a second slot (read)"
        );
        ...
        assertEq(
            ...
            0,
            "config spilled into a second slot (write)"
        );
    }
        ...
        if (shouldBond) {
            assertEq(
                ...
                amount *
                    BOND_BPS /
                    10_000,
                "bonded swap took the wrong bond"
            );
        } else {
            assertEq(
                bond,
                0,
                "swap below its own currency's threshold was bonded"
            );
        }
    }
}