...
contract BondCustodyTest is Test, Deployers {
    ...
    function test_bondedSwap_exactCustodyAccounting()
        public
    {
        uint256 swapperIn0 =
        ...
        uint256 managerGained =
            currency0.balanceOf(address(manager)) -
            managerIn0;

        uint256 swapperReceived =
            currency1.balanceOf(address(this)) -
            swapperIn1;
        ...
        assertEq(
            swapperSpent,
            BONDED_GROSS,
            "swapper did not pay exactly gross input"
        );
        ...
        assertEq(
            hookGained,
            EXPECTED_BOND,
            "hook did not take exactly the bond"
        );

        // The pool receives the trader's gross input minus the bond.
        assertEq(
            managerGained,
            BONDED_GROSS - EXPECTED_BOND,
            "pool did not receive gross - bond"
        );

        // Every input token paid by the trader is accounted for.
        assertEq(
            hookGained + managerGained,
            swapperSpent,
            "input tokens leaked"
        );
    ...
    function test_bondedSwap_hookHoldsNoResidualClaim()
        public
    {
        _swap(
            BONDED_INPUT,
    ...
    function test_bondedSwap_oneForZero_bondsInCurrency1()
        public
    {
        uint256 hookIn0 =
            currency0.balanceOf(address(hook));
        ...
        assertEq(
            currency1.balanceOf(address(hook)) - hookIn1,
            EXPECTED_BOND,
        ...
        assertEq(
            currency0.balanceOf(address(hook)),
            hookIn0,
            "currency0 balance moved on a oneForZero swap"
        );
    }
    ...
    function test_partialFill_bondIsSizedOffRequestedInput()
        public
    {
        uint256 swapperIn0 =
            currency0.balanceOf(address(this));
        ...
        (uint160 sqrtPriceNow,,,) =
            manager.getSlot0(id_);

        uint160 tightLimit =
            sqrtPriceNow -
            sqrtPriceNow / 1_000_000;
        ...
        assertLt(
            filled,
            BONDED_GROSS - EXPECTED_BOND,
            "swap did not partially fill; test is not exercising the case"
        );
        ...
        assertEq(
            hookGained,
            EXPECTED_BOND,
            "bond was not sized off the requested gross input"
        );

        // Trader spends only the actual filled input plus the already-taken bond.
        assertEq(
            swapperSpent,
            filled + EXPECTED_BOND,
            "outflow is not filled + bond"
        );

        assertLt(
            swapperSpent,
            BONDED_GROSS,
            "outflow should be below gross input on a partial fill"
        );
        ...
        assertEq(
            hookGained + filled,
            swapperSpent,
            "input tokens leaked on a partial fill"
        );
    }
    ...
    function test_invNoOp_bondEqualToGrossInput_reverts()
        public
    {
        ...
        vm.expectRevert(
            _wrapped(
                abi.encodeWithSelector(
                    BondMeBro.BondViolatesNoOpBound.selector,
                    BONDED_GROSS,
                    BONDED_GROSS
                )
            )
    ...
    function test_invNoOp_bondGreaterThanGrossInput_reverts()
        public
    {
    ...
    function test_invNoOp_bondRoundingToZero_reverts()
        public
    {
    ...
    ///
    /// `0 < bond < grossInput`
    function testFuzz_invNoOp_holdsForEveryValidConfig(
        uint128 minBonded,
        uint16 bondBps,
        uint96 grossInput
    )
        public
    {
        ...
        vm.assume(
            gross * bondBps / 10_000 > 0
        );

        uint256 expectedBond =
            gross * bondBps / 10_000;

        assertGt(
            expectedBond,
            0,
            "INV-NOOP lower bound violated: bond == 0"
        );

        assertLt(
            expectedBond,
            gross,
            "INV-NOOP upper bound violated: bond >= grossInput"
        );
    }
        ...
        internal
    {
        ...
        assertEq(
            storedMin0,
            MIN_BONDED,
            "vm.store wrote the wrong slot (minBondedAmount0)"
        );

        assertEq(
            storedMin1,
            MIN_BONDED_1,
            "vm.store wrote the wrong slot (minBondedAmount1)"
        );

        assertEq(
            storedBps,
            bondBps,
            "vm.store wrote the wrong slot (bondBps)"
        );
    }
    ...
    function test_bondExceedsTraderMax_reverts()
        public
    {
        uint128 tooLow =
            uint128(
    ...
    function test_bondEqualToTraderMax_succeeds()
        public
    {
        uint256 hookIn0 =
            currency0.balanceOf(address(hook));
        ...
        assertEq(
            currency0.balanceOf(address(hook)) -
                hookIn0,
            EXPECTED_BOND,
            "exact-ceiling swap did not bond"
        );
    }
    ...
    function test_bondedSwap_missingHookData_reverts()
        public
    {
        vm.expectRevert(
            _wrapped(
    ...
    function test_bondedSwap_wrongVersionHookData_reverts()
        public
    {
    ...
    function test_bondedSwap_truncatedHookData_reverts()
        public
    {
        bytes memory malformed =
            abi.encodePacked(
    ...
    function test_bondedSwap_zeroRecipientHookData_reverts()
        public
    {
        bytes memory malformed =
            abi.encodePacked(
        ...
        vm.expectRevert(
            _wrapped(
                abi.encodeWithSelector(
                    HookDataCodec.ZeroRefundRecipient.selector
                )
            )
        );
    ...
    function test_bondedSwap_missingHookData_movesNoTokens()
        public
    {
        uint256 hookBefore =
            currency0.balanceOf(address(hook));

        uint256 swapperBefore =
            currency0.balanceOf(address(this));
        ...
        assertEq(
            currency0.balanceOf(address(this)),
            swapperBefore,
            "swapper lost tokens on a reverted swap"
        );
    }
    ...
    function test_unbondedSwap_belowThreshold_emptyHookData_proceeds()
        public
    {
        uint256 hookBefore =
            currency0.balanceOf(address(hook));

        uint256 swapperBefore1 =
            currency1.balanceOf(address(this));
        ...
        assertEq(
            currency0.balanceOf(address(hook)),
            hookBefore,
            "unbonded swap moved tokens to the hook"
        );

        assertGt(
            currency1.balanceOf(address(this)),
            swapperBefore1,
            "unbonded swap produced no output"
        );
        ...
        assertEq(
            hook.lastTickBefore(),
            0,
            "beforeSwap did not record the pre-swap tick"
        );

        assertLt(
            hook.lastTickAfter(),
            0,
            "afterSwap did not record the post-swap tick"
        );
    }

    /// @notice A swap exactly equal to the threshold is bonded.
    function test_swapExactlyAtThreshold_isBonded()
        public
    {
        uint256 hookBefore =
            currency0.balanceOf(address(hook));
    ...
    function test_swapOneBelowThreshold_isUnbonded()
        public
    {
        uint256 hookBefore =
            currency0.balanceOf(address(hook));
        ...
        assertEq(
            currency0.balanceOf(address(hook)),
            hookBefore,
            "swap below the threshold was bonded"
        );
    }

    /// @notice A pool with bonding disabled never takes collateral.
    function test_unconfiguredPool_neverBonds()
        public
    {
        hook.setPoolConfig(
            key_,
    ...
    function test_exactOutput_onBondingPool_requiresHookData()
        public
    {
        vm.expectRevert(
            _wrapped(
    ...
    function test_exactOutput_onDisabledPool_needsNoHookData()
        public
    {
        hook.setPoolConfig(
            key_,
        ...
        assertEq(
            currency0.balanceOf(address(hook)),
            hookBefore0,
            "disabled pool bonded currency0"
        );

        assertEq(
            currency1.balanceOf(address(hook)),
            hookBefore1,
            "disabled pool bonded currency1"
        );
    }
    ...
    function test_setPoolConfig_rejectsBondBpsAboveCap()
        public
    {
        vm.expectRevert(
            abi.encodeWithSelector(
                BondMeBro.BondBpsAboveCap.selector,
                uint16(101),
                uint16(100)
            )
        );
    ...
    function test_setPoolConfig_acceptsBondBpsAtCap()
        public
    {
        hook.setPoolConfig(
            key_,
        ...
        assertEq(
            bondBps,
            100,
            "config at the cap was rejected"
        );
    }
    ...
    function test_setPoolConfig_rejectsPartialConfigurations()
        public
    {
        // Only threshold0 set.
        vm.expectRevert(
            abi.encodeWithSelector(
                BondMeBro.IncompletePoolConfig.selector,
                MIN_BONDED,
                uint96(0),
                uint16(0)
            )
    ...
    function test_setPoolConfig_allZeroDisables()
        public
    {
        hook.setPoolConfig(
            key_,
        ...
        assertEq(
            min0,
            0,
            "currency0 threshold not cleared"
        );

        assertEq(
            min1,
            0,
            "currency1 threshold not cleared"
        );

        assertEq(
            bondBps,
            0,
            "bond rate not cleared"
        );
    }

    /// @notice Deployment rejects a zero owner because ownership is immutable and nobody could configure pools afterward.
    function test_constructor_rejectsZeroOwner()
        public
    {
        (, bytes32 salt) =
            HookMiner.find(
    ...
    function test_setPoolConfig_onlyOwner()
        public
    {
    ...
    function test_directCallback_reverts()
        public
    {
        SwapParams memory params =
            SwapParams({
                zeroForOne: true,
                amountSpecified: BONDED_INPUT,
                sqrtPriceLimitX96:
                    TickMath.MIN_SQRT_PRICE + 1
            });
    ...
    function test_gas_unbondedSwap()
        public
    {
        _swap(
            UNBONDED_INPUT,
