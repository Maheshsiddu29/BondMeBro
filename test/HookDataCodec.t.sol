...
contract HookDataCodecHarness {
        ...
        )
    {
        ...
        (refundRecipient, maxBondAmount) =
            HookDataCodec.decode(data);
...
contract HookDataCodecTest is Test {
    ...
    uint128 internal constant MAX_BOND =
        1_000e18;
    ...
    uint256 internal constant DECODE_GAS_CEILING =
        1_500;

    function setUp() public {
        harness =
            new HookDataCodecHarness();
    }
        ...
        view
    {
        ...
        assertEq(
            recipient,
            RECIPIENT,
            "recipient did not survive the roundtrip"
        );

        assertEq(
            maxBond,
            MAX_BOND,
            "maxBondAmount did not survive the roundtrip"
        );
    }
    ...
    function test_wireFormat_versionByteIsFirst()
        public
        pure
    {
        bytes memory data =
        ...
        assertEq(
            ...
            "payload is not 37 bytes"
        );

        assertEq(
            uint8(data[0]),
            HookDataCodec.VERSION,
            "version byte is not at offset 0"
        );

        assertEq(
            HookDataCodec.ENCODED_LENGTH,
            37,
            "ENCODED_LENGTH drifted from 1 + 20 + 16"
        );
    }

    /// @notice Verifies the exact packed position of every version 1 field.
    function test_wireFormat_fieldOffsets()
        public
        view
    {
        address recipient =
        ...
        assertEq(
            ...
            "packed layout changed"
        );

        (
            address decodedRecipient,
            uint128 decodedMaxBond
        ) = harness.decode(data);
        ...
        view
    {
        ...
        assertEq(
            recipient,
            maxAddress,
            "all-ones address mangled"
        );

        assertEq(
            decodedMaxBond,
            maxBond,
            "type(uint128).max mangled"
        );
    ...
    function test_truncated_reverts()
        public
    {
        bytes memory valid =
            HookDataCodec.encode(
        ...
        for (
            uint256 len = 1;
            len < HookDataCodec.ENCODED_LENGTH;
            len++
        ) {
    ...
    function test_overLong_reverts()
        public
    {
        bytes memory valid =
            HookDataCodec.encode(
    ...
    function test_abiEncodedPayload_revertsAsWrongVersion()
        public
    {
        bytes memory abiEncoded =
            abi.encode(
                uint8(1),
                RECIPIENT,
                MAX_BOND
        ...
        assertEq(
            abiEncoded.length,
            96,
            "sanity: abi.encode of these three fields is 96 bytes"
        );

        assertEq(
            uint8(abiEncoded[0]),
            0,
            "sanity: abi.encode leaves byte 0 as padding"
        );
    ...
    function test_wrongVersion_reverts()
        public
    {
        bytes memory data =
            abi.encodePacked(
    ...
    function test_versionZero_reverts()
        public
    {
        bytes memory data =
            abi.encodePacked(
    ...
    function test_wrongVersion_isCheckedBeforeLength()
        public
    {
        bytes memory shortV2 =
            abi.encodePacked(
                uint8(2),
                bytes4(0xDEADBEEF)
            );
    ...
    function test_zeroRecipient_reverts()
        public
    {
        bytes memory data =
            abi.encodePacked(
    ...
    function test_zeroMaxBondAmount_reverts()
        public
    {
        bytes memory data =
            abi.encodePacked(
    ...
    function test_encode_rejectsZeroRecipient()
        public
    {
        vm.expectRevert(
            HookDataCodec
    ...
    function test_encode_rejectsZeroMaxBondAmount()
        public
    {
        vm.expectRevert(
            HookDataCodec
        ...
        view
    {
        ...
        assertEq(
            data.length,
            HookDataCodec.ENCODED_LENGTH,
            "encoded length is not constant"
        );
        ...
        assertEq(
            decodedRecipient,
            refundRecipient,
            "recipient not preserved"
        );

        assertEq(
            decodedMaxBond,
            maxBondAmount,
            "maxBondAmount not preserved"
        );
    }
    ...
    function testFuzz_wrongLengthAlwaysReverts(
        bytes calldata blob
    )
        public
    {
        ...
        vm.assume(
            blob.length !=
                HookDataCodec.ENCODED_LENGTH - 1
        );
    ...
    function testFuzz_anyNonOneVersionReverts(
        uint8 version,
        bytes calldata tail
    )
        public
        ...
        view
    {
        ...
        console2.log(
            "HookDataCodec.decode gas (isolated):",
            gasUsed
        );
        ...
        assertLt(
            gasUsed,
            DECODE_GAS_CEILING,
            "decode gas regressed past its fence"
        );
    }
}