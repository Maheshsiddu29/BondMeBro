...
///      SWAP_EXACT_IN_SINGLE, SETTLE, TAKE. The 32-byte hookData carries TRADER through
///      the router so a bond refund and piggyback reward go to the end user rather than
///      the router contract. Set a non-zero SWAP_AMOUNT_OUT_MINIMUM outside a demo.
///
...
/// - ZERO_FOR_ONE, SWAP_AMOUNT_IN, SWAP_AMOUNT_OUT_MINIMUM
/// Optional:
...
contract SwapBondMeBro is Script {
    ...
    function _request() internal view returns (SwapRequest memory request) {
        ...
        request.amountIn = uint128(vm.envUint("SWAP_AMOUNT_IN"));
        request.amountOutMinimum = uint128(vm.envUint("SWAP_AMOUNT_OUT_MINIMUM"));
        ...
        returns (bytes memory commands, bytes[] memory inputs)
    {
        ...
        IV4Router.ExactInputSingleParams memory swapParams = IV4Router.ExactInputSingleParams({
            ...
            minHopPriceX36: 0,
            hookData: abi.encode(request.trader)
        });
        ...
        actionParams[2] = abi.encode(outputCurrency, ActionConstants.MSG_SENDER, uint256(ActionConstants.OPEN_DELTA));
    ...
    function _approveInput(Currency currency, address permit2, address routerAddress) internal {
        ...
        IAllowanceTransfer(permit2)
            .approve(Currency.unwrap(currency), routerAddress, type(uint160).max, type(uint48).max);
    }
